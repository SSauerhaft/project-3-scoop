...
//Make createComment function for 'POST'
function createComment(url, request) {
  const requestComment = request.body && request.body.comment;
  const response = {};

  //Create new comment if comment has a body,and the username and
  // the articleId exist in the requestComment, and in the database
  if (requestComment &&
      requestComment.body &&
    ...
    //add newly created comment to database
    database.comments[comment.id] = comment;
    //add commentId to database.users.commentIds
    database.users[comment.username].commentIds.push(comment.id);
    //add commentId to database.articles.commentIds
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    const articleCommentIds = database.articles[id].commentIds;
